...
Несколько страниц на одном хабе — в разных браузерах или на разных компьютерах
локальной сети — делят одну руку: см. [«Несколько страниц»](#несколько-страниц-локальная-сеть-и-общая-рука).
...
|---|---|---|---|
| `move_all` | любой → страница / рука | `angles` — массив по осям J1…Jn (`null` — не трогать), `open` — раскрытие 0…100; `peer`, `name` — если шлёт соседняя страница (тогда действует замок) | страница ставит суставы (с ограничениями диапазона и пола) и рассылает новую позу остальным каналам |
...
| `hello` | страница → страницы | `peer`, `name` | новая страница представилась; каждая соседняя отвечает `peer` |
| `peer` | страница → страницы | `peer`, `name`, `components`, `lock` | ответ на `hello` (состав комнаты, кто держит замок и сколько ему осталось, мс) или смена имени |
| `arm` | страница → страницы | `peer`, `name`, `components` | владелец замка перестроил руку — соседи перестраивают свою (с «Отменой») |
| `peer_left` | хаб / страница → страницы | `peer`, `name` | участник выбыл, его замок снят |
...
## Несколько страниц: локальная сеть и общая рука

Две и больше страниц на одном хабе делят одну руку: состав и поза общие, управляет
тот, кто первый начал.

**Раздать страницу по сети.** На компьютере-хосте:

```bash
node tools/twin-mcp.mjs --serve
```

Хаб склеивает `index.html` + `src/` в один файл в памяти (как `tools/bundle.mjs`),
слушает все интерфейсы и печатает ссылки вида `http://192.168.1.5:8765/?twin=auto`.
На другом компьютере или телефоне в той же сети достаточно открыть ссылку — страница
подключается к хабу сама: `?twin=auto` — тот же хост, что у страницы, `?twin=ws://хост:порт` —
явный адрес; корень `/` без параметров перенаправляет на `?twin=auto`. Исходники `src/`
наружу не отдаются. `--host` задаёт адрес явно (по умолчанию `0.0.0.0` с `--serve` и
`127.0.0.1` без него); `--arm` работает как обычно — железо получает позу от того, кто
держит замок. Со страницы на `https://` к `ws://` не подключиться, поэтому по сети
раздаётся именно http-версия.

Без `--serve` — вручную: `node tools/twin-mcp.mjs --host 0.0.0.0`, а на второй машине
открыть свою копию `index.html` и ввести во вкладке `ws://192.168.1.5:8765`.

**Замок.** У каждой страницы случайный id (`peer`) и имя (поле «Ваше имя», хранится в
браузере). Поза (`move_all`, `gripper`) и состав (`arm`) уходят с полями `peer`/`name`.
Кто первый начал двигать руку или перестраивать её, тот держит замок: у остальных панель
деталей, кнопки добавления, Apply, IK-мишень и «В исходное» заблокированы, над 3D-видом —
плашка «🔒 Рукой управляет Анна · свободна через 1.4 с», а их 3D-рука повторяет действия
владельца. Замок продлевается каждым действием и отпускается через 2 с (`TWIN_LOCK_MS`)
после последнего. Срок считается по локальным часам получателя от момента приёма, поэтому
часы машин синхронизировать не нужно. Двое схватили руку в одно окно — побеждает меньший
`peer`, второй уступает и видит плашку. Замок живёт на страницах, хаб только пересылает;
MCP и железо в нём не участвуют — их команды все страницы применяют одинаково (они
приходят каждой через хаб).

**Вход.** Подключившись, страница шлёт `hello`; каждая соседняя отвечает `peer` со
своим составом и текущим замком. Новичок принимает состав комнаты (перестройка с
«Отменой»); если комната пуста (первый ответивший без деталей) — рассылает свой. Нет
ответа 1.5 с — комната его. Одновременный вход двух страниц: комната остаётся за меньшим
`peer`. Пока есть соседи, авто-анимация выключена — иначе она держала бы замок вечно.
При обрыве страницы хаб рассылает `peer_left` (закрывающаяся вкладка шлёт его и сама):
участник исчезает из списка «Страницы», его замок снимается.

Сообщения только между страницами (`hello`, `peer`, `peer_left`, `arm` с `peer`)
помечены `source:"3d"` — хаб не пересылает их железу; `move_all`/`gripper` с `peer`
идут и железу, лишние поля прошивка игнорирует.
...
Флаги: `--port` (8765), `--arm ws://…` (мост к руке), `--serve` (раздать страницу по
сети), `--host` (адрес; `0.0.0.0` с `--serve`, иначе `127.0.0.1`).

## Отладка

- Журнал вкладки: `→` ушло, `←` пришло, `?` не JSON, `!` ошибка канала, `•` подключения,
  `⊘` действие соседа отклонено (руку держит другой).
- Соседняя страница не видит мои движения: у меня плашка «Рукой управляет …»? Тогда
  замок у другого — подождите 2 с после его последнего действия. Список «Страницы»
  пуст — страницы подключены к разным хабам (проверьте адрес и порт).
