...
Переключатель рядом с темой меняет вид деталей: **Каркас** (цилиндры и коробки), **Гладкий**
(матовые печатные звенья с фасками, утопленными панелями, ступицами, крепежом и болтовым
фланцем основания) и **Корпус** (глянцевые белые сегментированные кожухи-капсулы,
металлические воротники и детализированные внешние моторы). Стиль меняет и оценку
3D-печати в BOM: строка печати считается по массе (граммы PLA из объёма деталей), под названием
руки — сводка «см³ / граммы / цена». Кинематика, URDF и коды ссылок от стиля не зависят;
`?style=` в ссылке выбирает его.
...
- **Локальная сеть, несколько страниц** — `node tools/twin-mcp.mjs --serve` раздаёт страницу по сети
  (`http://<ваш-ip>:8765/`); все браузеры и компьютеры, открывшие её, делят одну руку — состав и позу.
  Кто первый начал двигать, тот держит руку, остальные зеркалят и могут перехватить через 2 с после
  последнего действия.
