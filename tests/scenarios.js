...
(async () => { try {
  ...
  if (SCEN === 'twin') {
    ...
    twinAddLink(fake);
    expect(sent.some(m => m.type === 'hello' && m.peer === twin.me.peer && m.source === '3d') && !twin.adopted, 'новому WebSocket-каналу — hello, состав комнаты ещё не решён');
    ...
    twinHandle('{"type":"peer","peer":"room0001","name":"Room","components":[]}', fake);
    expect(twin.adopted && sent.some(m => m.type === 'arm' && m.peer === twin.me.peer && m.components.length === 5), 'комната пуста — свой состав ушёл соседям');
    expect(twin.peers.has('room0001') && /Room/.test(twinPeersEl.textContent), 'сосед в списке страниц');
    sent.length = 0;
    twinTick(performance.now(), true);
    expect(sent.some(m => m.type === 'move_all' && m.angles.join() === '0,0,0' && m.peer === twin.me.peer) && sent.some(m => m.type === 'gripper' && m.open === 50), 'после входа поза уходит целиком, с peer');
    sent.length = 0;
    ...
    /* --- соседние страницы: замок на 2 с, гонка по id, состав, hello/peer/peer_left --- */
    const t0 = performance.now();
    twin.lock = null; sent.length = 0;
    twinHandle('{"type":"move_all","angles":[33],"peer":"aaaa0001","name":"Anna"}', fake);
    expect(components[0].angle === 33 && twin.lock?.peer === 'aaaa0001' && document.body.classList.contains('twinLocked'), 'поза соседа применена, замок его, панель заблокирована');
    expect(!twinLockHint.hidden && /Anna/.test(twinLockHint.textContent) && /🔒 Anna/.test(twinPeersEl.textContent), 'плашка и список с именем владельца');
    twinTick(t0 + 100);
    expect(!sent.some(m => m.type === 'move_all'), 'принятая от соседа поза не уходит обратно');
    twinHandle('{"type":"move_all","angles":[44],"peer":"bbbb0002","name":"Boris"}', fake);
    expect(components[0].angle === 33 && twin.lock.peer === 'aaaa0001', 'второй сосед при чужом замке игнорируется');
    setParamChecked(components[0], 'angle', 10); sent.length = 0; twinTick(t0 + 200);
    expect(!sent.length, 'при чужом замке своя поза не уходит');
    twinTick(t0 + TWIN_LOCK_MS + 300);
    expect(!twin.lock && !document.body.classList.contains('twinLocked') && twinLockHint.hidden, 'через 2 с после последнего действия замок снят');
    setParamChecked(components[0], 'angle', 20); sent.length = 0; twinTick(t0 + TWIN_LOCK_MS + 400);
    expect(twin.lock?.peer === twin.me.peer && sent.some(m => m.type === 'move_all' && m.peer === twin.me.peer && m.angles[0] === 20), 'своё действие берёт замок, move_all уходит с peer');
    twinHandle('{"type":"move_all","angles":[55],"peer":"zzzz","name":"Zed"}', fake);
    expect(components[0].angle === 20 && twin.lock.peer === twin.me.peer, 'гонка: сосед с большим id мой замок не перехватывает');
    twinHandle('{"type":"move_all","angles":[66],"peer":"0","name":"Ann"}', fake);
    expect(components[0].angle === 66 && twin.lock.peer === '0', 'гонка: соседу с меньшим id уступаю');
    twinHandle(JSON.stringify({ type: 'arm', peer: '0', name: 'Ann', components: [{ type: 'yaw', angle: 5 }, { type: 'link', length: 1 }, { type: 'gripper', open: 30 }] }), fake);
    expect(components.length === 3 && components[0].angle === 5 && components[2].open === 30, 'состав соседа принят вместе с позой');
    sent.length = 0; twinTick(t0 + TWIN_LOCK_MS + 500);
    expect(!sent.some(m => m.type === 'arm' || m.type === 'move_all'), 'принятые состав и поза не уходят обратно');
    twinHandle('{"type":"peer_left","peer":"0"}', fake);
    expect(!twin.lock && !twin.peers.has('0') && !document.body.classList.contains('twinLocked'), 'peer_left: участник выбыл, его замок снят');
    sent.length = 0;
    twinHandle('{"type":"hello","peer":"cccc0003","name":"Carl"}', fake);
    const pr = sent.find(m => m.type === 'peer');
    expect(pr && pr.peer === twin.me.peer && pr.components.length === 3 && twin.peers.has('cccc0003'), 'hello: ответ peer со своим составом');
    twinHandle(JSON.stringify({ type: 'move_all', angles: [99], peer: twin.me.peer }), fake);
    expect(components[0].angle === 5, 'своё эхо игнорируется');
    sent.length = 0; twin.lock = null;
    setArm([...cleanConfig(), { type: 'roll', angle: 0 }]);
    expect(sent.some(m => m.type === 'arm' && m.peer === twin.me.peer && m.source === '3d' && m.components.length === 4) && twin.lock?.peer === twin.me.peer, 'своя перестройка → arm соседям, замок мой');
    twinRemoveLink(fake);
    expect(!twin.peers.size, 'без WebSocket список соседей пуст');
    twinAddLink(fake);
    twinHandle('{"type":"peer","peer":"room0001","name":"Room","components":[{"type":"pitch","angle":10}]}', fake);
    expect(twin.adopted && components.length === 1 && components[0].type === 'pitch' && components[0].angle === 10, 'новичок принимает состав комнаты');
    twin.adopted = false; twinHandle('{"type":"hello","peer":"zzzz"}', fake);
    expect(twin.adopted, 'одновременный вход: комната остаётся за меньшим id');
    twin.adopted = false; twinHandle('{"type":"hello","peer":"0"}', fake);
    expect(!twin.adopted, 'одновременный вход: у соседа id меньше — жду его состав');
    twinAdopted();
    twin.lock = null; twinRenderLock();
